...
void lcdDrawHLine(uint16_t x0, uint16_t x1, uint16_t y, uint16_t color)
{
  ...
  lcdSetCursorPosition(x0, y, x1, y);
...
void lcdDrawVLine(uint16_t x, uint16_t y0, uint16_t y1, uint16_t color)
{
  ...
  lcdSetCursorPosition(x, y0, x, y1);
  ...
  // if ((x >= lcdProperties.width) || (y0 >= lcdProperties.height || y1 < 1)) return;
  // if ((y0 + y1 - 1) >= lcdProperties.height)
  //   y1 = lcdProperties.height - y0;
  // if (y1 < 2 ) {
	//   lcdDrawPixel(x, y0, color);		
	//   return;
  // }
}
...
static void lcdDrawPixels(uint16_t x, uint16_t y, uint16_t *data, uint32_t dataLength)
{
  ...
  lcdSetCursorPosition(x, y, lcdProperties.width - 1, lcdProperties.height - 1);
